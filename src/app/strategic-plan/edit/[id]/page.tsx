                    ...
                    <CardContent className="space-y-6">
                        ...
                        <Accordion type="multiple" className="w-full space-y-4" defaultValue={pillarFields.map(p => p.id || '')}>
                            {pillarFields.map((pillar, pIndex) => (
                                <PillarAccordion key={pillar.id} pIndex={pIndex} form={form} removePillar={() => removePillar(pIndex)} />
                            ))}
                        </Accordion>
...
function PillarAccordion({ pIndex, form, removePillar }: { pIndex: number; form: any; removePillar: () => void; }) {
    ...
    return (
        <AccordionItem value={form.getValues(`pillars.${pIndex}.objectives.${oIndex}.id`)}>
            ...
            <AccordionContent className="p-4 border border-t-0 rounded-b-md space-y-4">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => appendInitiative({ id: generateId('I'), title: ``, description: "", owner: "Liam Johnson", collaborators: [], activities: [] })}>
